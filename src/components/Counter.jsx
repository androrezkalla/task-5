...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case 'decrement':
      return { ...state, count: state.count - state.numberToChangeBy };
    case 'updateCount':
      return { ...state, numberToChangeBy: parseInt(action.payload, 10) };
    default:
      return state;
  }
};
  ...
  return (
    <div className="App">
      ...
      <div className="flex gap center">
        ...
        <p className="flex gap center"><label className="button-box" htmlFor="number">Number to Increment/Decrement by</label><input className="input-box"onChange={(e) => dispatch({ type: 'updateCount', payload: e.target.value })}type="number" value={state.numberToChangeBy} name="number" id="number"/></p>
    </div>);
};
